...
from app.services.langchain_chat import complete_json
...
class EvidenceBoundGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            structured_method: prompt 兼容输出或 json_mode 服务端 JSON 输出。
        """
    ...
    def _safe_error_detail(self, error: Exception) -> str:
        ...
        return str(error) if isinstance(error, ModelCallError) else type(error).__name__
    ...
    def _call_llm(
        ...
        result = complete_json(
            ...
            stage="consultation-generation",
            structured_method=self.structured_method,
        )
        result = self._parse_json_content(json.dumps(result, ensure_ascii=False))
